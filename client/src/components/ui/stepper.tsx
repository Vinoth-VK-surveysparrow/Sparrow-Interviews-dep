...
function StepperTrigger({ asChild = false, className, children, tabIndex, ...props }: StepperTriggerProps) {
  ...
  return (
    <button
      ...
      className={cn(
        'cursor-pointer focus-visible:border-ring focus-visible:ring-ring/50 inline-flex items-center gap-3 rounded-full outline-none focus-visible:z-10 focus-visible:ring-[3px] disabled:pointer-events-none disabled:opacity-60 disabled:cursor-not-allowed',
        className,
      )}
      onClick={() => !isDisabled && setActiveStep(step)}
      onKeyDown={(e) => !isDisabled && handleKeyDown(e)}
      disabled={isDisabled}
      {...props}
    >
      {children}
